...
%    025_step14_detect_badch_psd3sd_qc.csv
%    025_step14_psdzprune_detect_qc.csv
%
%  IMPORTANT:
...
for f = 1:length(segment_files)
    try
        ...
        % 519_step13_remove_channels_muscleICmask_segment_timepoint_0-timepoint_30_trimmed25
...
% -------- Bands (Hz) --------
bands = struct( ...
    'delta', [0.5 4], ...     % start at 0.5 Hz if the high-pass and data quality allow
    'theta', [4 8], ...
    'alpha', [8 13], ...
    'beta',  [13 30], ...
...
function [lo,hi]=default_span_from_name(name_lower)
    ...
    elseif contains(n,'delta'), lo=0.5; hi=4;
...
% Goal:
% For each CONDITION, make 5 panels (delta/theta/alpha/beta/gamma).
...
requiredCols = {'famlbnr','CBCL_totsc_cg_v15','CGASx_v15'};
...
vars_to_numeric = {'CBCL_totsc_cg_v15','CGASx_v15'};
...
% Keep only needed columns
Tclin = Tclin(:, {'Subject','CBCL_totsc_cg_v15','CGASx_v15'});
...
clinical_labels = containers.Map( ...
    {'CBCL_totsc_cg_v15','CGASx_v15'}, ...
    {'CBCL total score','CGAS'} );
...
for ci = 1:numel(conds)
    ...
    for vi = 1:numel(clinical_vars)
        ...
        if numel(x_all) > 3 && numel(unique(x_all)) > 1
            ...
            legend_labels{end+1} = 'Linear fit'; %#ok<SAGROW>

            % Top-left annotation
            txtLP = sprintf('\\rho = %.2f, p = %.3f', rho, pval);
        ...
        if ~isempty(legend_handles)
            ...
            % Manually placed to sit below the top-right text
            lgd.Units = 'normalized';
            lgd.Position = [0.762 0.78 0.14 0.14];
        end
